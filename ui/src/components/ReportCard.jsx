...
import { FiEdit, FiDownload, FiExternalLink, FiCheck, FiX } from "react-icons/fi";
...
function Donut({ data, total, dark }) {
  ...
  const SIZE = 100, R = 36, cx = SIZE / 2, cy = SIZE / 2;
  ...
  const passedPct = total > 0 ? Math.round(((data.passed || 0) / total) * 100) : 0;
  const hovSeg = hov ? segments.find(s => s.k === hov) : null;

  if (total === 0) return (
    <svg width={SIZE} height={SIZE}>
      <circle cx={cx} cy={cy} r={R} fill="none" stroke={dark ? "#1e293b" : "#e2e8f0"} strokeWidth={10} />
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle"
        fontSize="12" fill={dark ? "#475569" : "#94a3b8"} fontFamily="monospace" fontWeight="700">N/A</text>
    </svg>
  );
  ...
  return (
    <div style={{ position: "relative", width: SIZE, height: SIZE, flexShrink: 0 }}>
      <svg width={SIZE} height={SIZE} style={{ transform: "rotate(-90deg)" }}>
        <circle cx={cx} cy={cy} r={R} fill="none" stroke={dark ? "#1e293b" : "#e2e8f0"} strokeWidth={10} />
        {segments.map(seg => (
          <circle key={seg.k} cx={cx} cy={cy} r={R} fill="none"
            stroke={seg.color}
            strokeWidth={hov === seg.k ? 14 : 10}
            strokeDasharray={`${seg.dash} ${seg.gap}`}
            strokeDashoffset={-seg.offset}
            strokeLinecap="butt"
            style={{ transition: "stroke-width 0.15s, opacity 0.15s", opacity: hov && hov !== seg.k ? 0.25 : 1, cursor: "pointer" }}
      ...
      <div style={{
        ...
        {hovSeg ? (
          <>
            <span style={{ fontSize: 17, fontWeight: 800, color: hovSeg.color, fontFamily: "monospace", lineHeight: 1.1 }}>{hovSeg.v}</span>
            <span style={{ fontSize: 9, color: dark ? "#64748b" : "#94a3b8", fontFamily: "monospace", textTransform: "uppercase", letterSpacing: "0.06em" }}>
              {S[hovSeg.k].label}
            </span>
          </>
        ) : (
          <>
            <span style={{ fontSize: 19, fontWeight: 800, color: dark ? "#f1f5f9" : "#0f172a", fontFamily: "monospace", lineHeight: 1.1 }}>{passedPct}%</span>
            <span style={{ fontSize: 9, color: dark ? "#475569" : "#94a3b8", fontFamily: "monospace" }}>pass rate</span>
          </>
        )}
      </div>
    </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{
        ...
        {Object.entries(data).map(([k, v]) => v > 0 && (
          <div key={k} title={`${S[k].label}: ${v}`}
            style={{ width: `${(v / total) * 100}%`, background: S[k].color, transition: "width 0.4s" }} />
        ))}
      </div>
      {/* Pills */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
        {Object.entries(data).map(([k, v]) => (
          <span key={k} style={{
            ...
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: S[k].color, flexShrink: 0 }} />
            <span style={{ fontSize: 14, fontWeight: 700, color: S[k].color, fontFamily: "monospace", marginBottom: 2 }}>{v}</span>
            <span style={{ fontSize: 13, color: dark ? "#94a3b8" : "#475569", fontFamily: "monospace" }}>{S[k].label}</span>
          </span>
        ))}
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <span style={{
        ...
        background: sc, boxShadow: `0 0 7px ${sc}`,
        flexShrink: 0,
        animation: isSuccess ? "pulseDot 2s infinite" : "none",
      }} />

      {/* CI name */}
      <span style={{ fontSize: 13, fontWeight: 600, color: ci.color, fontFamily: "monospace", flex: 1 }}>
        {ci.label}
      </span>
      ...
      {bn && (
        <span style={{
          fontSize: 12, fontWeight: 700, color: sc, marginBottom: 3,
          fontFamily: "monospace",
          background: `${sc}18`, padding: "1px 7px", borderRadius: 6,
        }}>
          {bn}
        </span>
      )}
...
// ── Main ReportCard ────────────────────────────────────────────────────────────
export default function ReportCard({ r, darkMode, editingReportId, editingName, setEditingName, setEditingReportId, handleSaveName }) {
  ...
  const hasPipelineStatus = status.pipeline_status === 0 || status.pipeline_status === 1;
const accent = hasPipelineStatus
  ? status.pipeline_status === 0 ? "#22c55e" : "#ef4444"
  : total === 0
    ? "#475569"
  ...
  return (
    <>
      ...
      <div className="rc" style={{
        ...
        borderRadius: 18,
        overflow: "hidden",
        boxShadow: dm ? "0 4px 24px rgba(0,0,0,0.5)" : "0 4px 20px rgba(0,0,0,0.08)",
        display: "flex",
        ...
        {/* Top accent bar */}
        <div style={{ height: 4, background: `linear-gradient(90deg, ${accent}, ${accent}33)` }} />
        ...
        <div style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {/* Breadcrumb */}
                <span style={{
                  fontSize: 13, fontWeight: 700, color: txtMuted,
              ...
              {editing ? (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <input value={editingName}
                    ...
                    style={{
                      ...
                      color: txtMain, fontSize: 15, fontWeight: 700,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                  <a
                    ...
                    style={{
                      fontSize: 18, fontWeight: 800, color: txtMain,
                  ...
                  <button className="rc-ed"
                    onClick={() => { setEditingReportId(r.id); setEditingName(r.name); }}
                    style={{ background: "none", border: "none", padding: 3, cursor: "pointer", color: txtMuted, display: "flex", flexShrink: 0 }}>
                    <FiEdit size={14} />
                  </button>
                  <a href={r.minio || r.minio_url} target="_blank" rel="noopener noreferrer"
                    className="rc-dl"
                    style={{ display: "flex", padding: 3, color: txtMuted, flexShrink: 0 }}
                    title="Download">
                    <FiDownload size={14} />
                  </a>
                </div>
              ...
              {/* Date */}
              <p style={{ fontSize: 14, color: txtMuted, marginTop: 5, fontFamily: "monospace" }}>
          ...
          {total > 0
            ...
            : <p style={{ fontSize: 13, color: txtMuted, fontFamily: "monospace", margin: 0 }}>No test data</p>
          }
